...
CULTURE_KEYWORDS = [
    ...
    # K팝·셀럽은 별도 목록(CELEB_KEYWORDS)에서 교차조건과 함께 처리 — 아래 참조.
...
CELEB_KEYWORDS = ["케이팝", "K팝", "아이돌", "그래미", "빌보드"]
# 순수 컴백/음반 홍보와 구분하는 교차 신호 — 기관·콜라보·타 장르·이슈일 때만 통과.
CELEB_CROSSOVER = [
    "미술관", "박물관", "갤러리", "국립", "아트", "전시", "작품",
    "콜라보", "협업", "뮷즈", "브로드웨이", "웨스트엔드", "보이콧", "논란",
]
...
def is_art_related(title: str) -> bool:
    ...
    if any(k in title for k in ART_KEYWORDS):
        return True
    # 문화 전 장르(문학·영상 IP·문화재·정책) — 단, '사건 동사'가 있을 때만.
    # 장르어만 있고 사건이 없으면 단순 소개글이라 우리 채널 소재가 아니다.
    if any(k in title for k in CULTURE_KEYWORDS) and EVENT_VERB_RE.search(title):
        return True
    # 케이팝·셀럽은 사건 동사만으로 부족하다 — 실사고(2026-08-25, 빅뱅 투어 개막·NCT127
    # 신보 발매 2건이 그대로 발행됨): 컴백·발매·투어·팬미팅은 전부 '사건 동사'를 달고
    # 나오는 순수 음악산업 홍보라서 EVENT_VERB_RE만으로는 못 거른다. 문화·기관 교차
    # 키워드가 함께 있을 때만(콜라보·기관 협업·타 장르 진출·수상/보이콧 이슈) 통과시킨다.
    if any(k in title for k in CELEB_KEYWORDS) and any(k in title for k in CELEB_CROSSOVER):
        return True
