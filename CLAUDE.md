...
## 서버 사이드 일일 자동화 (노트북 꺼도 동작)

| 항목 | 내용 |
|------|------|
| 워크플로 | `.github/workflows/daily-news.yml` |
| 셀렉터 | `tools/daily_auto.py` (RSS 기반, og:image 검증 포함) |
| 실행 시간 | 매일 12:00 KST (03:00 UTC) — cron `0 3 * * *` |
| 수동 트리거 | GitHub → Actions → "Daily News Auto" → Run workflow |
| Secret | `DISCORD_WEBHOOK_URL` (레포 Settings → Secrets) |

**동작:** 뉴스1/중앙일보/매경/디자인프레스 RSS 순회 → 아트 키워드 필터 → 블랙리스트 도메인 제외 → og:image HEAD 검증 → 첫 통과 후보로 JSON 생성 → `discord_notify_ci.py` 실행. 회색 이미지 방지를 위해 검증 실패 후보는 자동 스킵.

> 사용자가 직접 Claude Code에서 뉴스 고르는 흐름은 그대로 유지된다. `daily-news.yml`은 **노트북 꺼진 시간대 백업용** 일일 발송이다. 같은 날 수동/자동이 겹치면 파일명 suffix (`_auto.json`)로 구분된다.
