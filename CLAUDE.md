...
### 폴백 자동 모드 (GitHub Actions — 매일 18:11/19:11/20:11/21:11 KST 시도)
...
**지각 컷오프**: Actions 스케줄이 밀려 KST 16:00~23:30 밖에 실행되면 발행하지 않고 그날은 거른다
(예약 지연이 7~8시간까지 벌어져 새벽 알림이 나간 사고 — 2026-08-29 05:58, 2026-09-01 02:13 KST).
그래서 크론을 한 시간 간격 4개로 걸어 한 타임이 밀려도 다른 타임이 그날 발행을 살린다.
...
| 폴백 | GitHub Actions `daily-news.yml` 18:11/19:11/20:11/21:11 KST 시도 (당일 발행물이 main 또는 `claude/*`에 있으면 스킵, KST 16:00~23:30 밖 실행이면 그날 거름) |
