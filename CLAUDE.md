...
- 합니다체, 정보 전달형 큐레이터 어투 (해요체·구어체 금지)
- 이미지 출처: `이미지 | © [원저작권자]` 기본. 인스타그램 원출처면 `이미지 | @[계정명]`
- 구조: 본문 → 📍장소 → 📅기간(있을 때) → `이미지 | ©` → 해시태그
- 자세한 규칙: `.claude/skills/daily-news/headline-caption.md`
