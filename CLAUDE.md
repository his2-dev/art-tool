...
**❌ 블랙리스트 (og:image 차단 or HTML 403):**
- `kh.or.kr` (국가유산진흥원) — 모든 페이지 차단
- `korea.kr` (정책브리핑) — 차단
- `*.go.kr` 정부 기관 도메인 전반
- `mmca.go.kr`, `sema.seoul.go.kr` 등 공공 미술관 공식 사이트
- `*.seoul.go.kr` 서울시 공공

**✅ 화이트리스트 (검증됨):**
- 헤럴드경제 `biz.heraldcorp.com`, `heraldcorp.com`
- 디자인프레스 `design.co.kr`
- 뉴스1 `news1.kr`
- 중앙일보 `joongang.co.kr`
- 매일경제 `mk.co.kr`
...
**절차:**
1. 공식 사이트에 쓸 만한 이미지가 있더라도 `image_url`에는 **해당 소식을 보도한 상업 언론사 기사 URL**을 넣을 것
2. `og:image`가 있는지, 해당 이미지가 실제 접근 가능한지 가급적 사전 확인
3. `source` 표기는 여전히 `© [원저작권자]` (예: `© 서울시립미술관`) — 이건 퀄리티와 별개로 유지
...
> **중요**: `image_url`에는 반드시 위 "이미지 URL 선별 규칙"의 **화이트리스트 언론사**를 사용할 것. `kh.or.kr`, `korea.kr` 같은 정부/공공 도메인은 CI에서 403으로 차단되어 회색 배경으로 폴백된다.
